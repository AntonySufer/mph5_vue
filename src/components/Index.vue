<template>
      <!--内容区域 S-->
<div class="shopmanmain wx_guanlian bghui lh40" id="Index" >
    <!--头部用户信息 S-->
                   ...
                   if (jsonData.returnCode =="success") {
                       ...
                       var  resultList = jsonData.orders;
                           if (resultList != null && resultList.length > 0) {
                                ...
                                _this.$router.go('/orderList',{query:{orally:'info'}}); //回到首页
                          }else{
                                 if (_this.relation_status) {
                                   _this.$router.go('/orderList',{query:{orally:'info'}}); //回到首页
                                 }else{
                                    _this.$router.go('/shopNoInfo',{query:{from:'orderList'}}); //回到首页
                ...
                data:{token:_this.accessToken,offset:0,limit:1}
                     ...
                     if (jsonData.returnCode =="success") {
                        ...
                        var resultList =jsonData.shops;
                           if (resultList != null && resultList.length > 0) {
                                 //跳到列表  入口来源orally=info
                               _this.$router.go('/shopList',{query:{orally:'info'}}); //回到首页
                          }else{
                                 if (_this.relation_status) {
                                    //跳到列表  入口来源orally=info
                                      _this.$router.go('/shopList',{query:{orally:'info'}}); //回到首页
                                 }else{
                                     _this.$router.go('/shopNoInfo',{query:{from:'shopList'}}); //回到首页
                                 }
                           }
                     }else{  
                        console.log("错误日志："+jsonData.returnCode);
                ...
                data:{token:_this.accessToken,offset:0,limit:1}
                     ...
                     if (jsonData.returnCode =="success") {
                         ...
                         var resultList = jsonData.cards;
                           if (resultList != null && resultList.length > 0) {
                              //跳到列表  入口来源orally=info
                                 _this.$router.go('/cardList',{query:{orally:'info'}}); //回到首页
                          }else{
                                 if (_this.relation_status) {
                                    //跳到列表  入口来源orally=info
                                     _this.$router.go('/cardList',{query:{orally:'info'}}); //回到首页
                                    ...
                                    _this.$router.go('/shopNoInfo',{query:{from:'cardList'}}); //回到首页
...
.hid_slh2{overflow : hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical;}
/*雪碧图*/
 #Index .sp{background: url("/resources/images/meirong/shop/20160718_yy_fwh/my_xf_sp.png") no-repeat;}
/*头部用户信息*/
#Index .vip_top_box{width: 640px; height: 200px; background: url("/resources/images/meirong/shop/20160718_yy_fwh/my_xf_top_bg.jpg") no-repeat;}
...
#Index .vip_myself_icon1{display: block; width: 46px; height: 46px; background-position: 0 -519px; margin: 21px 0 0 0; float: left;}
#Index .vip_myself_icon2{display: block; width: 46px; height: 46px; background-position: -53px -519px; margin: 21px 0 0 0; float: left;}
#Index .vip_myself_icon3{display: block; width: 46px; height: 46px; background-position: -115px -519px; margin: 21px 0 0 0; float: left;}
#Index .vip_myself_icon4{display: block; width: 46px; height: 46px; background-position: -175px -519px; margin: 21px 0 0 0; float: left;}

#Index .vip_myself_s2{display: block; width: 514px; height: 46px; background-position: 0 -519px; margin: 21px 0 0 0; text-indent: 16px; color: #7c7c7c; font-size: 30px; line-height: 46px; float: left;}
#Index .vip_myself_s3{display: block; width: 20px; height: 34px; color: #b3b3b3; font-size: 40px; text-align: center; line-height: 10000px; float: left; background: url("/resources/images/meirong/shop/20160718_yy_fwh/jt_sp.png") -105px -1px no-repeat; margin-top: 26px;}
